...
export const tuiEn: TuiDictionary = {
  ...
  turnRunning: (role, elapsed) => `running ${role} · ${elapsed}`,
  turnStarted: (role) => `Running ${role}… Cancel with Ctrl+C.`,
  turnSucceeded: (role) => `${role} finished the turn.`,
  turnFailed: (role) => `${role}'s turn failed.`,
  turnCancelActiveHint: "Cancel turn (Ctrl+C)",
  runRoleNeedsRole: "Choose a role to run. Members of this room:",
  runRoleAvailableRoles: (roles) => `Members of this room: ${roles}.`,
  runRoleNoMembers: "This room has no role members to run.",
  confirmYesNo: "Type y to confirm or n to cancel.",
  confirmTypeRoleId: (roleId) => `Type ${roleId} to confirm, or cancel to abort.`,
  confirmTypeWorldId: (worldId) => `Type ${worldId} to confirm, or cancel to abort.`,
  simTickConfirmPrompt: (world, ticks) =>
    `Advance world ${world} by ${ticks} ticks? This writes world state and cannot be automatically undone.`,
  simForkConfirmPrompt: (world, label) =>
    `Fork world ${world} into branch ${label}? This writes a new world to disk.`,
  simIrreversibleNote: "The runtime cannot automatically revert this.",
  simActionCancelled: "Simulation action cancelled.",
  modelChangePrompt: (current, next) => `Change default model from ${current} to ${next}?`,
  modelChangeCancelled: "Model change cancelled.",
  modelChanged: (providerModel) => `Default model updated to ${providerModel}.`,
  rollbackNeedsHistoryId: "Rollback needs a history id: :rollback <history-id>.",
  rollbackHint: "Type :rollback to undo this apply.",
  configRolledBack: (historyId, paths) => `Config rolled back to ${historyId}. Restored: ${paths}.`,
  slashRollbackDescription: "roll config back to a history entry",
  ...
  roleNotInRoom: (roleLabel, roomName) => `${roleLabel} is not a member of ${roomName}.`,
