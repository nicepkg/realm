...
export const tuiZhCn: TuiDictionary = {
  ...
  turnRunning: (role, elapsed) => `${role} 运行中 · ${elapsed}`,
  turnStarted: (role) => `${role} 回合运行中……按 Ctrl+C 取消。`,
  turnSucceeded: (role) => `${role} 已完成回合。`,
  turnFailed: (role) => `${role} 的回合失败了。`,
  turnCancelActiveHint: "取消回合（Ctrl+C）",
  runRoleNeedsRole: "请选择要运行的角色。本房间成员：",
  runRoleAvailableRoles: (roles) => `本房间成员：${roles}。`,
  runRoleNoMembers: "本房间没有可运行的角色成员。",
  ...
  roleNotInRoom: (roleLabel, roomName) => `${roleLabel} 不在房间 ${roomName} 中。`,
  ...
  simForkConfirmPrompt: (world, label) =>
    `将世界 ${world} 分叉为分支 ${label}？这将向磁盘写入新世界。`,
  simIrreversibleNote: "运行时无法自动撤销此操作。",
  simActionCancelled: "已取消模拟操作。",
  modelChangePrompt: (current, next) => `将默认模型从 ${current} 改为 ${next}？`,
  modelChangeCancelled: "已取消模型变更。",
  modelChanged: (providerModel) => `默认模型已更新为 ${providerModel}。`,
  rollbackNeedsHistoryId: "回滚需要历史 id：:rollback <history-id>。",
  rollbackHint: "输入 :rollback 可撤销本次应用。",
  configRolledBack: (historyId, paths) => `配置已回滚至 ${historyId}。已恢复：${paths}。`,
  slashRollbackDescription: "将配置回滚到某个历史记录",
  room: "房间",
